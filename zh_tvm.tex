...
\begin{abstract}
本文的目的是描述 Telegram Open Network Virtual Machine（TON VM 或 TVM），它用於在 TON 區塊鏈上執行智能合約。
\end{abstract}

\section*{Introduction}
\markbothsame{Introduction}

Telegram Open Network Virtual Machine（TON VM 或 TVM）的主要目的是在 TON 區塊鏈上執行智能合約程式碼。TVM 必須支持解析傳入訊息和持久性數據所需的所有操作，以及創建新訊息和修改持久性數據的操作。

此外，TVM 必須滿足以下要求：
\begin{itemize}
\item 它必須提供可能的未來擴展和改進，同時保持向後兼容性和互操作性，因為智能合約的程式碼一旦提交到區塊鏈上，就必須在不考慮 VM 的任何未來修改的情況下繼續按照可預測的方式運作。
\item 它必須努力實現高“(虛擬)機器碼”密度，以便一個典型的智能合約的程式碼佔用的持久性區塊鏈存儲盡可能少。
\item 它必須完全是可確定性的。換句話說，使用相同的輸入數據運行同樣的程式碼必須產生相同的結果，而不考慮具體使用的軟件和硬件。\footnote{例如，TVM 中不存在浮點數算術運算（這可以在大多數現代 CPU 上使用硬體支援的{\em double}類型高效實現），因為執行這些運算的結果取決於具體的底層硬體實現和捨入模式設置。相反，TVM 支持特殊的整數算術運算，如果需要，可以用來模擬定點算術。}
\end{itemize}

TVM 的設計是受這些要求的指導。儘管本文描述的是 TVM 的初步和實驗性版本，\footnote{在正式發佈之前，生產版本可能需要進行一些微調和修改，這些只有在使用測試環境中的實驗性版本一段時間後才會變得明顯。} 但系統內建的向後兼容性機制使我們相對不用擔心在這個初步版本中用於 TVM 程式碼的操作編碼的效率。

TVM 不打算以硬體形式實現（例如，使用專門的微處理器芯片）；相反，它應該在常規硬體上運行的軟件中實現。這一考慮讓我們在 TVM 中包含一些高層次的概念和操作，這些在硬體實現中需要繁複的微程式，但對軟件實現沒有太大問題。當智能合約程式碼部署在 TON 區塊鏈上時，這些操作對於實現高程式碼密度和最小化智能合約程式碼的位元（或存儲單元）配置非常有用。
